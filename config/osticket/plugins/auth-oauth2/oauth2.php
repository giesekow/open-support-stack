...
trait OAuth2AuthenticationTrait {
    ...
    function callback($resp, $ref=null) {
        try {
            $expectedState = $this->getState();
            $incomingState = isset($resp['state']) ? $resp['state'] : null;
...
abstract class OAuth2ProviderBackend extends OAuth2AuthorizationBackend {
    ...
    public function getToken($code, $getResourceOwner = true) {
        ...
        if ($getResourceOwner && $this->getResourceOwnerUrl()) {
            try {
                if (($owner=$this->getResourceOwner($token))
                ...
                // Some IdPs (including certain Keycloak responses) can return
                // non-JSON from userinfo on verification edge-cases.
                // Fall back to JWT token claims so auth can still proceed.
                $fallback = $this->mapAttributes($token->getOwnerAttributes());
                if ($fallback)
                    $token->setOwner((Object) array_merge($token->getOwnerAttributes(), $fallback));
            }
        }
        return $token;
    }
    ...
    public function getOwner() {
        if (!isset($this->owner)) {
            ...
            if (is_object($jwt)) {
                ...
                $username = $jwt->preferred_username ?? $jwt->username ?? null;
                $givenname = $jwt->given_name ?? null;
                $surname = $jwt->family_name ?? null;
            }
            if (!$email)
                $email = $this->getUniqueName() ?: $this->getUpn();
            if (!$username)
                $username = $email ?: ($this->getUniqueName() ?: $this->getUpn());
            $this->owner = (object) [
                'id' =>  $jwt->sub ?? $jwt->oid ?? null,
                'email' => $email,
                'username' => $username,
                'givenname' => $givenname,
                'surname' => $surname,
