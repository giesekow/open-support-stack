...
- `NGINX_MODE=https` (or `http` if TLS is terminated upstream)
...
- For automatic public certs, enable (only when `NGINX_MODE=https`):
...
- If `NGINX_MODE=https` and Let's Encrypt is disabled, nginx falls back to local certs in `nginx/certs/`.
- If `NGINX_MODE=http`, certbot stays idle and no certificates are issued by this stack.
